...
import { GastosItems } from "../Components/gastosItems";
...
const Gastos = () => {
  ...
  const onSearchValueChange = (event) => {
    gastos.setSearchValue(event.target.value);
  };
      ...
      <div className=" p-4 h-full">
        <div className="p-5 bg-white container h-full rounded-lg space-y-4">
          ...
          <div className="overflow-y-scroll h-5/6">
            {gastos.searchedGasto.length > 0 ? (
              <table className="w-full  ">
                <thead className="bg-gray-800 w-full p-4">
                  <tr className="text-white grid grid-cols-6 p-4">
                    <th className="text-sm w-1/6">Id</th>
                    <th className="text-sm w-1/6">Fecha</th>
                    <th className="text-sm w-1/6">Motivo</th>
                    <th className="text-sm  w-1/6">Valor</th>
                    <th className="text-sm w-1/6">Acciones</th>
                  </tr>
                </thead>
                <tbody className="" id="tbody">
                  {gastos.searchedGasto.map((Gasto) => (
                    <GastosItems item={Gasto} key={Gasto.id} />
                  ))}
                </tbody>
              </table>
            ) : (
